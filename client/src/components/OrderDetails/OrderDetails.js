...
const OrderDetails = () => {
  ...
  useEffect(() => {
    const fetchOrdersAndRevenue = async () => {
      ...
      `;
      try {
        const [ordersResponse, orderStatusData] = await Promise.all([graphQLCommand(ordersQuery), graphQLCommand(orderStatusQuery)]);
        console.log("Fetched Orders:", ordersResponse.getAllOrders);
          ...
          {filteredOrders.length > 0 ? (
            filteredOrders.map((order) => (
              <div key={order.id} className="bg-primaryBlack p-6 rounded-lg shadow-xl hover:shadow-2xl transition-all duration-300 text-white">
                ...
                <p className="text-sm font-semibold text-gray-400 mb-2">Discount: ₹{calculateDiscount(order.cart).toFixed(2)}</p> {/* Total Discount */}
                ...
                {expandedOrder === order.id && (
                  <div className="mt-4 space-y-2">
                    {order.cart.length > 0 ? (
                      ...
                      <p className="text-center text-gray-500">No products found in cart</p>
                    )}
                  </div>
                )}
              </div>
